...
import { uploadImageProfile } from "../handlers/imageHandler";
import { getUserDataFromDatabase, updateUserDatabaseField } from "../handlers/userHandler";
...
export const useProfileSettings = () => {
  ...
  const handleSave = async (fieldKey: string) => {
    const success = await updateUserDatabaseField(fieldKey, tempValue);
    if (success) {
      setUserData({ ...userData, [fieldKey]: tempValue });
      setActiveField(null);
    } else {
  ...
  const handleUpdateProfileImage = async () => {
    const imageUri = await gallery.pickImage();
    ...
    if (imageUri) {
        ...
        try {
            const downloadUrl = await uploadImageProfile(userId, imageUri);
            await updateUserDatabaseField("imageUrl", downloadUrl);
  ...
  return {
    username: userData?.username || dummyProfileData.username,
    email: userData?.email || dummyProfileData.email,
    phoneNumber: userData?.phoneNumber || dummyProfileData.phoneNumber,
    imageUrl: userData?.imageUrl || dummyProfileData.imageUrl,
    verified: userData?.verified || dummyProfileData.verified,
